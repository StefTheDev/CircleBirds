...
bool GameScene::Load()
{
	...
	for (int i = 0; i < 3; i++)
	{
		std::shared_ptr<PhysicsEntity> cube = std::make_shared<PhysicsEntity>(*world, BOX_ENTITY, SDL_Rect{ 16 * i, 16 * i, 80, 7 }, false, textureManager->GetTexture(PLANK_WOOD_SPRITE));

		b2RevoluteJointDef revoluteJointDef;
		if (i == 0) {
			revoluteJointDef.localAnchorA.Set(0, -4.0);
			revoluteJointDef.localAnchorB.Set(-0.4, 0);
		}
		else {
